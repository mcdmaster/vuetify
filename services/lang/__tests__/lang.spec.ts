...
describe.skip('$vuetify.lang', () => {
  ...
  it('should fall back to en', () => {
    ...
    expect('Translation key "bar" not found, falling back to default').toHaveBeenCalled()

    expect(lang.t('$vuetify.baz')).toBe('$vuetify.baz')
    expect('Translation key "baz" not found, falling back to default').toHaveBeenCalled()
    expect('Translation key "baz" not found in fallback').toHaveBeenCalled()
  })
