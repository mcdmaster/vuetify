...
describe.skip('VTreeView.ts', () => { // eslint-disable-line max-statements
  ...
  it.skip('should load children when expanding', async () => {
    ...
    expect(wrapper.html()).toMatchSnapshot()
    expect(`[Vue warn]: Error in created hook: "TypeError: Cannot set property 'vnode' of undefined"`).toHaveBeenCalled()
    expect(`TypeError: Cannot set property 'vnode' of undefined`).toHaveBeenCalled()
  })
  ...
  it('should warn developer when using non-scoped slots', () => {
    ...
    expect('[Vuetify] The prepend and append slots require a slot-scope attribute').toHaveBeenCalled()
  })
