...
describe('VIcon', () => {
  ...
  it('should support an icon name in the default slot', () => {
    const wrapper = mount(VIcon, {
      slots: {
        default: () => h(typeof Text, null, 'md:add'),
      },
      global: globalOptions,
    })
  ...
  it('should render a specific size', () => {
    ...
    expect(wrapper.element.getAttribute.arguments.fontSize).toBe('112px')
  })
  ...
  it('should render a component with aria-hidden attr', () => {
    ...
    expect(wrapper.element.getAttribute('aria-hidden')).toContain('foo')
  })
