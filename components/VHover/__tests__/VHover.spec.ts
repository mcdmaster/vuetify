...
describe.skip('VHover.ts', () => {
  ...
  it('should warn when missing scoped slot and bound value', () => {
    ...
    expect('v-hover is missing a default scopedSlot or bound value').toHaveBeenCalled()
  })
  ...
  it('should warn when using multiple root elements', () => {
    ...
    expect('v-hover should only contain a single element').toHaveBeenCalled()
    expect('[Vue warn]: Multiple root nodes returned from render function. Render function should return a single root node.').toHaveBeenCalled()
  })
})
